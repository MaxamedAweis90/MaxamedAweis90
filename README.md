# 👋 Hello, I'm Mohamed Aweys
 
- 💻 **Full-Stack Developer** | **Flutter** | **Next.js** | **Firebase**
- 🎓 IT Student at **SIMAD University**
- 🌍 Passionate about creating user-friendly web and mobile apps
- 🚀 Currently building **SheekoChat** (A Web & Mobile chat app using Firebase & Supabase)
- 🌱 I’m always learning new tech and growing as a developer

## 🛠️ Tech Stack & Tools

- **Languages**: JavaScript, Dart (Flutter), HTML, CSS
- **Frameworks**: React.js, Next.js, Flutter
- **Databases**: Firebase Firestore, Supabase
- **Tools**: GitHub, Git, VS Code, Figma, Postman, VS Code

## 📈 GitHub Stats
![Mohamed's GitHub Stats](https://github-readme-stats.vercel.app/api?username=MaxamedAweis90&show_icons=true&theme=radical)

## 🧠 Currently Learning
- Flutter (Advanced) 🏆
- MongoDB 🗃️
- GraphQL 📡

## 📫 How to reach me:
- [LinkedIn](your-link)
- [Twitter](your-link)
- [Portfolio](your-link)

## 🔗 Featured Projects:
- [SheekoChat](link-to-project) – A real-time chat application with Firebase and Supabase.
- [Portfolio](link-to-portfolio) – Personal portfolio with Next.js and Sanity.

## 👯 Let's Connect:
- [LinkedIn](your-link)
- [Twitter](your-link)
